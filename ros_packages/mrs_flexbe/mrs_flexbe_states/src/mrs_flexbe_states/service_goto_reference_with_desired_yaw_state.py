...
class ServiceGoToReferenceWithDesiredYawState(EventState):
    '''
    ...
    def __init__(self, service_topic,control_manager_diagnostics_topic):
        ...
        super(ServiceGoToReferenceWithDesiredYawState, self).__init__(input_keys=['goal_tracker_point', 'desired_yaw', 'frame_id'],
        ...
        if msg_topic == self._control_manager_diagnostics_topic:
            ...
            Logger.loginfo('[GoToReferenceWithDesiredYawState]: Successfully subscribed to topic %s' % self._control_manager_diagnostics_topic)
    ...
    def execute(self, userdata):
        ...
        if not self._connected:
            Logger.logerr('[GoToReferenceWithDesiredYawState]: not connected to %s' % self._control_manager_diagnostics_topic)
            return 'failed'
        
        diff_time = rospy.get_time() - self._start_time
        
        if self._sub_cont_diag.has_msg(self._control_manager_diagnostics_topic):
            rospy.loginfo_throttle(2,'[GoToReferenceWithDesiredYawState]: has_diagnostics msg at time %s' % (str(rospy.get_time())))
    ...
    def on_enter(self, userdata):
        ...
        if not self._connected:
            ...
            if msg_topic == self._control_manager_diagnostics_topic:
                ...
                self._sub_cont_diag = ProxySubscriberCached({self._control_manager_diagnostics_topic: msg_type})
                self._connected = True
                Logger.loginfo('[GoToReferenceWithDesiredYawState]: Successfully subscribed to previously unavailable topic %s' % self._control_manager_diagnostics_topic)
            else:
                Logger.logwarn('[GoToReferenceWithDesiredYawState]: Topic %s still not available, giving up.' % self._control_manager_diagnostics_topic)

        if self._connected and self._sub_cont_diag.has_msg(self._control_manager_diagnostics_topic):
            Logger.loginfo('[GoToReferenceWithDesiredYawState]: Waiting for msg from topic %s' % self._control_manager_diagnostics_topic)
            self._sub_cont_diag.remove_last_msg(self._control_manager_diagnostics_topic)
        ...
        if not self._srv.is_available(self._service_topic):
            Logger.logerr('[GoToReferenceWithDesiredYawState]: Topic \'{}\' not yet registered!'.format(self._service_topic))
            self._failed = True
            return
        ...
        try:
            ...
            service_request.reference.heading = userdata.desired_yaw
            service_result = self._srv.call(self._service_topic, service_request)

            if not service_result.success:
                self._failed = True
                Logger.logwarn('[GoToReferenceWithDesiredYawState]: Calling \'{}\' was not successful: {} for reference position (x,y,z) = ({},{},{})'.format(self._service_topic, str(service_result.message),service_request.reference.position.x,service_request.reference.position.y,service_request.reference.position.z))
            else:
                Logger.loginfo('[GoToReferenceWithDesiredYawState]: {}'.format(str(service_result.message)))

        except Exception as e:
            Logger.logerr('[GoToReferenceWithDesiredYawState]: Failed to call \'{}\' service request: {}'.format(self._service_topic, str(e)))
            self._failed = True

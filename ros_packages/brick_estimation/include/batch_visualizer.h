...
public:
  BatchVisualizer();
  ~BatchVisualizer();
  BatchVisualizer(ros::NodeHandle &rosNode, std::string frame, std::string topic);
  ...
  void clear();
  void publish(const int id, const bool filled);
...
BatchVisualizer::BatchVisualizer(ros::NodeHandle &rosNode, std::string frame, const std::string topic) {

  this->frame      = frame;
  this->visual_pub = rosNode.advertise<visualization_msgs::Marker>(topic, 1);
}
//}
...
/* publish */  //{
void BatchVisualizer::publish(const int id, const bool filled) {
